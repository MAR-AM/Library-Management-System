...
class LibraryManagementSystem:
    ...
    def add_data(self):
        if self.TypeMember.get()=='':
            ...
            my_cursor.execute("insert into library_data values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                            self.TypeMember.get(),
    ...
    def get_cursor(self,event=""):
        ...
        self.Book_id_entry.set(row[8]),
        self.Book_Title_entry.set(row[9]),
        self.Auther_Name_entry.set(row[10]),
        self.nbr_page_entry.set(row[11]),
        self.Edition_entry.set(row[12]),
        self.Date_purchase_entry.set(row[13]),
        self.Quantity_entry.set(row[14]),
        self.Price_entry.set(row[15])
    ...
    def delete(self):
        if self.Code_CIN_entry.get() == "" or self.Book_id_entry.get() == "":
            messagebox.showwarning("Error", "First select the member")
        else:
            Connect = mysql.connector.connect(host="localhost", user="Mama", password="Mama@123@", database="Library")
            my_cursor = Connect.cursor()
            query = "DELETE FROM library_data WHERE Code_CIN = %s AND Book_id = %s"
            values = (self.Code_CIN_entry.get(), self.Book_id_entry.get())
    ...
    def Update(self):
        if self.Code_CIN_entry.get() == "" or self.Book_id_entry.get() == "":
            ...
            Connect = mysql.connector.connect(host="localhost", user="Mama", password="Mama@123@", database="Library")
            my_cursor = Connect.cursor()
            query = "UPDATE library_data SET Member_type = %s, First_name = %s, Last_Name = %s, Gender = %s, Adresse = %s, Postal_Code = %s, Code_CIN = %s, Mobile_Number = %s, Book_id = %s, Book_Title = %s, Auther_Name = %s, nbr_page = %s, Edition = %s, Date_purchase = %s, Quantity = %s, Price = %s WHERE Code_CIN = %s"
            values = (
                self.TypeMember.get(),
                self.First_name_entry.get(),
                self.last_name_entry.get(),
